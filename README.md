...
Statistical Insights: Computes the average number of words per sentence.

Word Frequency & Length: Identifies the shortest and longest words along with their frequencies (probabilities).

Alphabetical Sorting: Automatically sorts words with the same frequency in alphabetical order.


🚀 Usage
The program runs via the Terminal (Command Line) and requires two parameters:

python3 text_analyzer.py input.txt output.txt

Input file (the text to be analyzed)
